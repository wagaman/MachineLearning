...
def parse_web_listing(url):
    ...
    for line in r.content.split('\n'):
        ...
        if match:
            ...
            elif match.group(1).lower().endswith(constants.SUPPORTED_EXTENSIONS):
...
def parse_folder(folder, labels_file,
    ...
    if train_file:
        ...
        output_dir = os.path.dirname(val_file)
    labels_idxfile = open(output_dir + '/' + constants.LABELS_INDEX_FILE, 'w')
    ...
    for subdir in subdirs:
        ...
        labels_idxfile.write(label_name + ' ' + str(label_index))
        if label_index < len(subdirs) - 1:
            labels_idxfile.write('\n')
